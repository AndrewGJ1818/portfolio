...
All fonts are served directly through the Google Fonts API to ensure great performance, automatic format delivery, and shared caching benefits across browsers. The descriptor `font-display: swap` is applied to all imported families, preventing text invisibility during loading and allowing an immediate fallback display for better perceived performance (FOUT strategy), while also avoiding font-loading strategies that might bypass the use of the custom typefaces, which are essential to preserving the site’s intended visual identity and overall aesthetic experience.

The following families are used throughout the project:

- **[Dynalight.](https://fonts.google.com/specimen/Dynalight)** A decorative script font used exclusively in the Home section, where it appears on the diploma illustration that introduces the site. Dynalight was selected after evaluating other calligraphic options whose uppercase characters felt overly ornate and hindered readability. Its smoother letterforms preserve a handwritten aesthetic while maintaining clear legibility, making it ideal for a signature-like display use. The `text=AndrewGacíiJ` parameter will be used to download only the required glyphs, reducing the font file size and improving initial render performance.
